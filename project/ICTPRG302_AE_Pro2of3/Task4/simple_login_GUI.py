...
def open_view_window():
    ...
    # create label for total users
    user3_var = tk.StringVar()
    label3 = tk.Label(new_window, textvariable=user3_var)
    user3_var.set("Total users = X")
    label3.pack()

    # create listbox window to show list of registered users
    ...
    if user_var.get() != "Hello, World!":
        with open(file_path, 'r') as file:
            ...
            total_usr = "Total users = " + str(index)
            user3_var.set(total_usr)
    else:
